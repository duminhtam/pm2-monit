  ...
  pm2.connect(function() {
    ...
    setInterval(function() {
      pm2.list(function(err, procs) {
        console.log('procs.length', procs.length);
      });
    }, 5000);
...
Monitor.prototype.quit = function () {
  console.log('Closing pm2 pub emitter socket.');
  this.pm2Sock && this.pm2Sock.close();
  console.log('Closing statsd dgram socket.');
  ...
  var timer = setTimeout(function () {
    pm.list(me.options.pm2Conf.DAEMON_RPC_PORT, function (err, procs) {
      ...
      if (!!procs) {
        ...
        console.log('Sent', 'system data is null');
      }
      clearTimeout(timer);
